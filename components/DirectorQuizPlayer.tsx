...
import { getDirectorQuizRound, POSTER_IMAGE_BASE_URL, DirectorQuizRoundData } from '../services/tmdbService';
import { useTranslation } from '../hooks/useTranslation';
import { MovieGenre } from '../types';
import { PlayerHeader } from './PlayerHeader';

interface DirectorQuizPlayerProps {
  onBack: () => void;
  genre: MovieGenre;
  onGameOver: (game: 'directorQuiz', genre: MovieGenre, score: number) => void;
}

export const DirectorQuizPlayer: React.FC<DirectorQuizPlayerProps> = ({ onBack, genre, onGameOver }) => {
  const { t } = useTranslation();
  const [score, setScore] = useState(0);
  const [roundData, setRoundData] = useState<DirectorQuizRoundData | null>(null);
  ...
  const fetchNextRound = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getDirectorQuizRound(genre);
      setRoundData(data);
    } catch (err) {
  ...
  const handleAnswerSelect = (selectedName: string) => {
    if (userChoice || !roundData) return;

    const { correctDirector } = roundData;
    const isCorrect = selectedName === correctDirector.name;
    setUserChoice({ name: selectedName, isCorrect });
    ...
    if (isCorrect) {
      ...
      onGameOver('directorQuiz', genre, score);
  ...
  const getButtonClass = (option: string) => {
    ...
    const { correctDirector } = roundData;
    const isCorrect = option === correctDirector.name;
    const isSelected = option === userChoice.name;
      ...
      {!isLoading && !error && roundData && (
        <div className="w-full max-w-2xl pt-28 sm:pt-16">
          <div className="flex flex-col items-center mb-8">
            <div className="relative w-40 sm:w-48 md:w-64 aspect-[2/3] rounded-xl shadow-2xl overflow-hidden mb-4 border-4 border-white dark:border-gray-600">
                <img 
            ...
            <p className="text-lg text-gray-500 dark:text-gray-400">{new Date(roundData.movie.release_date).getFullYear()}</p>
          </div>
