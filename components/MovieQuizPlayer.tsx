...
import { getMovieQuizRound, IMAGE_BASE_URL, Movie } from '../services/tmdbService';
import { useTranslation } from '../hooks/useTranslation';
import { MovieGenre } from '../types';
import { PlayerHeader } from './PlayerHeader';

interface MovieQuizPlayerProps {
  onBack: () => void;
  genre: MovieGenre;
  onGameOver: (game: 'movieQuiz', genre: MovieGenre, score: number) => void;
}
...
export const MovieQuizPlayer: React.FC<MovieQuizPlayerProps> = ({ onBack, genre, onGameOver }) => {
  ...
  const fetchNextRound = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getMovieQuizRound(genre);
      setRoundData(data);
    } catch (err) {
  ...
  const handleAnswerSelect = (selectedTitle: string) => {
    ...
    if (isCorrect) {
      ...
      onGameOver('movieQuiz', genre, score);
      ...
      {!isLoading && !error && roundData && (
        <div className="w-full max-w-4xl pt-24 sm:pt-0">
          <div className="relative w-full aspect-video rounded-xl shadow-2xl overflow-hidden mb-8 border-4 border-white dark:border-gray-600">
            <img 
                key={roundData.correctMovie.id}
                src={`${IMAGE_BASE_URL}${roundData.correctMovie.backdrop_path}`} 
                alt="Movie backdrop" 
