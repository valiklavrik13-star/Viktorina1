...
import { getActorQuizRound, POSTER_IMAGE_BASE_URL, ActorQuizRoundData } from '../services/tmdbService';
import { useTranslation } from '../hooks/useTranslation';
import { MovieGenre } from '../types';
import { PlayerHeader } from './PlayerHeader';

const ActorList: React.FC<{ actors: ActorQuizRoundData['actors'] }> = ({ actors }) => {
    const { t } = useTranslation();
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700 mb-8">
            <h2 className="text-xl font-bold text-center text-gray-800 dark:text-white mb-4">{t('actorQuiz.starring')}</h2>
...
interface ActorQuizPlayerProps {
  onBack: () => void;
  genre: MovieGenre;
  onGameOver: (game: 'actorQuiz', genre: MovieGenre, score: number) => void;
}

export const ActorQuizPlayer: React.FC<ActorQuizPlayerProps> = ({ onBack, genre, onGameOver }) => {
  const { t } = useTranslation();
  const [score, setScore] = useState(0);
  const [roundData, setRoundData] = useState<ActorQuizRoundData | null>(null);
  ...
  const fetchNextRound = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getActorQuizRound(genre);
      setRoundData(data);
    } catch (err) {
  ...
  const handleAnswerSelect = (selectedTitle: string) => {
    ...
    if (isCorrect) {
      ...
      onGameOver('actorQuiz', genre, score);
