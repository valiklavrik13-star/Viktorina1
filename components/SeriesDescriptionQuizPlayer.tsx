...
import { getSeriesDescriptionQuizRound, SeriesDescriptionQuizRoundData } from '../services/tmdbService';
...
interface SeriesDescriptionQuizPlayerProps {
  onBack: () => void;
  genre: MovieGenre;
  onGameOver: (game: 'seriesDescriptionQuiz', genre: MovieGenre, result: { rounds: number, avgPercentage: number }) => void;
}

export const SeriesDescriptionQuizPlayer: React.FC<SeriesDescriptionQuizPlayerProps> = ({ onBack, genre, onGameOver }) => {
  const { t } = useTranslation();
  const [score, setScore] = useState(0); // This now represents rounds won
  const [roundData, setRoundData] = useState<SeriesDescriptionQuizRoundData | null>(null);
  ...
  const triggerGameOver = useCallback(() => {
    ...
    onGameOver('seriesDescriptionQuiz', genre, { rounds: score, avgPercentage });
    setIsGameOver(true);
  ...
  const fetchNextRound = useCallback(async () => {
    ...
    setBudget(100);
    try {
      const data = await getSeriesDescriptionQuizRound(genre);
      setRoundData(data);
      const wordArray = data.correctSeries.overview?.split(/\s+/) || [];
